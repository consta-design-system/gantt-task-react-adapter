...
const convertDateToString = (date: Date) =>
  `${prependZero(date.getDate())}.${prependZero(date.getMonth() + 1)}.${date.getFullYear()}`
...
export const TooltipContent = (props: Props) => {
  ...
  return (
    <Card verticalSpace="s" horizontalSpace="m" id={id} className={cnToolripContent()}>
      {type === 'task' && project && (
      ...
      {type !== 'milestone' ? (
        <>
          <Text size="s">Продолжительность: {`${days} ${getDaysPostfix(days)}`}</Text>
